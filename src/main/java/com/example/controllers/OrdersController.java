...
public class OrdersController {
    ...
    public void sendEmail(Users user, Orders order) {
        ...
        try {
            ...
            System.out.println("Total prep time order.getStatus(): " + order.getStatus());
            helper.setText(" Order Status- ", order.getStatus()+"\n" +" Order Quantity- " +String.valueOf(order.getQuantity())+"\n" +"\n" +" Order Date- " +order.getDate()+"\n" +" Order Time- " +order.getTime());
            ...
            if (checkOfficeHours(sqlTime)) {
                if (checkTimeAppropriate(sqlTime, sqlDate, session)) {
                    while (orderListIterator.hasNext()) {
                        ...
                        System.out.println("Total prep time singleorders: " + singleorders);
                        System.out.println("Total prep time orders: " + orders);
...
//                return new ModelAndView("picktime", "error", "Pick up time not within office hours. Please choose between 6am-9pm. Earliest pickup time is: " + earlyTime);
                return "pickUpTimeNotWithinOfficeHours";            }
        }
        else{
            Date dm = new Date();
...
//            return new ModelAndView("picktime", "error", "Did not choose time! Earliest pickup time is: " + earlyTime);
            return "\"Did not choose time! Earliest pickup time is: \" + earlyTime";
        }
    }
    ...
    public void sendPickUpMail(Users user, String time) {
        MimeMessage mail = javaMailSender.createMimeMessage();
        try {
            MimeMessageHelper helper = new MimeMessageHelper(mail, true);
            ...
            helper.setReplyTo("cmpe275rwba@gmail.com");
            helper.setFrom("cmpe275rwba@gmail.com");
            helper.setSubject("Your Order is Ready To Pick Up");
            helper.setText(" Your Order will be ready in -", time);
    ...
    public void changeStatus() {
    	//int userid = (Integer) session.getAttribute("id");
    	System.out.println("Status set: Processing! Order globaluserid: " + globaluserid);
    	Users user = userService.findById(globaluserid);
            ...
            if (calendarA.getTime().after(calendarB.getTime())) {
                ...
                while (orderListIterator.hasNext() && !done) {
                    ...
                    if (od.getordersid()==singleorders.getOrdersid() && od.getStatus().equals("Order Placed")) {
                        ...
                        String readyTime = preparationTime+ "Mins"; 
                        System.out.println("Status set: Processing! Order readyTime: " + user.getEmail());
                        sendPickUpMail(user, readyTime);
